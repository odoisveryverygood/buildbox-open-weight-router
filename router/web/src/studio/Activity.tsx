...
export default function Activity({runId}: {runId?:string}) {
  ...
  const [summary,setSummary]=useState<S['UsageSummary']>(),[route,setRoute]=useState(''),[since,setSince]=useState(''),[until,setUntil]=useState(''),[intermediate,setIntermediate]=useState<S['StoredOutput'][]>([]);
  ...
  async function load() {
    ...
    try {const next=await request<S['SandboxRun']>(`/api/sandbox/runs/${id}`);setRun(next); const q=new URLSearchParams(location.search);q.set('run',next.id);history.replaceState(null,'',`?${q}`);
      ...
      setIntermediate(await request<S['StoredOutput'][]>(`/api/sandbox/runs/${next.id}/outputs`));
  ...
  return <section id="activity"><h2>Activity & usage</h2><p>Open an exact persisted run. Opening or refreshing is read-only; replaying is a new explicitly costed action in Routes.</p><div className="form-grid"><label>Run ID<input value={id} onChange={e=>setId(e.target.value)} /></label><button className="secondary" disabled={busy||!identifier.test(id)} onClick={load}>Load / refresh run</button></div>
    ...
    <details><summary>Aggregated model-attempt usage (chat and workflows)</summary><div className="form-grid"><label>Alias or policy ID filter<input value={route} onChange={e=>setRoute(e.target.value)}/></label><label>From UTC date<input type="date" value={since} onChange={e=>setSince(e.target.value)}/></label><label>Through UTC date<input type="date" value={until} onChange={e=>setUntil(e.target.value)}/></label></div><button className="secondary" onClick={async()=>{try{setSummary(await request<S['UsageSummary']>(`/api/studio/usage?${new URLSearchParams({route,since,until})}`));}catch(e){setError(String(e));}}}>Refresh usage without running</button>{summary&&<><p>{summary.attempts.length} attempts · known subtotal {money(summary.known_subtotal_micro_usd)} · total {money(summary.actual_micro_usd)} · reserved {money(summary.reserved_micro_usd)} · unresolved {summary.unresolved_attempts}.</p><p>Coverage: at most 1,000 latest attempt records; not provider billing. Includes failed attempts, not just successful answers. Planning spending is shown separately above.</p><ul>{summary.attempts.map(a=><li key={a.id}>{a.trace.alias_id??refKey(a.trace.policy)} · {a.status} · {a.trace.configuration_id} · actual {money(a.usage.actual_micro_usd)}</li>)}</ul></>}</details>
    ...
    {run&&<><p role="status">{run.id} · {status} · {refKey(run.policy)} · {run.quality}</p><p>Created {run.created_at} · persisted attempts {attempts.length}. Failed attempts count; unknown usage is not free.</p>
      ...
      <details><summary>Persisted stage outputs ({intermediate.length})</summary>{intermediate.map(o=><article key={o.id}><h4>{o.node_id}</h4><pre>{JSON.stringify(o.value,null,2)}</pre><p>Recorded {o.created_at}; expires {o.expires_at}. Source content is untrusted data, not tool authority.</p></article>)}</details>
