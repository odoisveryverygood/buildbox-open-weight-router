...
export default function Studio({view, dirty}: {view:S['PlanView']; dirty:boolean}) {
  ...
  const [versions,setVersions]=useState<S['VersionRef'][]>([]);
  ...
  return <section className="studio"><div className="section-head"><h2>Workflow studio</h2><span className="badge">Sandbox only · quality provisional</span></div>
    ...
    {tab==='steps'&&<><p>Compile only a saved workflow. Proposed prompts and text types below are editable defaults for review, not discovered requirements. Incomplete bindings must be corrected in the planner; unsupported stages stay blocked.</p>
      ...
      {saved&&<EditPolicy value={saved} onProposal={p=>{update(p);setReview(true);}}/>}
      {saved&&<details><summary>Version history / explicit rollback</summary><button className="secondary" onClick={()=>void act(async()=>setVersions((await request<S['PolicyHistory']>(`/api/studio/policies/${saved.policy.id}/history`)).versions))}>Load persisted history</button>{versions.map(v=><button className="secondary" key={refKey(v)} onClick={()=>void act(async()=>{accept(await request<S['PolicyView']>(policyPath(v)));setDraft(undefined);})}>Inspect {refKey(v)}</button>)}<p>Inspection never switches traffic. To roll back, inspect the old version and explicitly enable it with a still-valid admission in Routes. Each alias remains pinned; disable the superseded version explicitly.</p></details>}
      {draft&&<fieldset disabled={busy}><h3>Draft {refKey(draft)} — never active</h3><p>{draft.stages.length} stages · {draft.budget.max_model_calls} bounded model calls · {draft.budget.max_tool_calls} bounded tool calls. Planning iteration limits are not assumed sample-call counts.</p>
        {draft.stages.map((stage,index)=>{
          ...
          return <article className="model-card" key={stage.node_id}><h3>{index+1}. {node.purpose}</h3><p>{node.kind} · depends on {node.depends_on?.join(', ')||'workflow input'} · configuration {stage.configuration_id??'No model'}</p>
            <p>Model pins use the server-validated edit proposal above. Editing a prompt never silently changes a model.</p>
            ...
            <p>Bindings: <code>{JSON.stringify(node.inputs)}</code></p><div className="form-grid">{Object.entries(stage.output_types).map(([key,type])=><label key={key}>Output {key} type<select value={type} onChange={e=>update({...draft,stages:draft.stages.map(s=>s===stage?{...s,output_types:{...s.output_types,[key]:e.target.value as typeof type}}:s)})}>{['text','json','boolean','number'].map(t=><option key={t}>{t}</option>)}</select></label>)}</div><p className="hint">Changing a type must preserve downstream typed bindings; the server rejects mismatches. Use the proposed-edit form to review a strict JSON output schema; unsupported schema keywords are rejected.</p>
          </article>;
        })}
      ...
      <details><summary>Current structured-edit limits</summary><p>Region, cost ceiling, API/self-host restriction, capabilities and graph edits create a new planning version. Pin/exclusion edits and targeted cheaper-stage instructions use the server proposal above. Arbitrary natural-language edits are not silently interpreted; ambiguous stage names need clarification. Quality priorities remain provisional heuristics.</p></details>
    </>}
