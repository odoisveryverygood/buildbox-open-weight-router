...
const initial: S['PlanInput'] = {schema_version:'1.0', intake: {schema_version:'1.0', example_id:null, tools:[], description:'', constraints: {schema_version:'1.0', open_weight_required:true, max_cost_per_1k_tokens:null, max_latency_ms:null, required_region:null, provenance:declared}}, processing:defaultsProcessing, requirements:defaultsRequirements, answers:[], catalog_mode:'fixture', edited_workflow:null, proposal_mode:'interpreter',runtime_catalog_id:null};
...
export default function PlanningApp() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (caps.mode === 'live') setDraft({...initial, catalog_mode: caps.runtime_catalog_ids?.length ? 'approved_runtime' : 'public_snapshot', runtime_catalog_id:caps.runtime_catalog_ids?.[0]??null});
  ...
  return <div className="shell">
    ...
    <main><header><span>BUILD / EXPLORE / VERIFY</span><span className="pill">{capabilities?.mode === 'fixture' ? 'FIXTURE MODE' : 'Permission-gated runtime'}</span></header>
      <p><a href="/?intelligence=1">Workload intelligence: inspect capability fit, execution strategy and policy tradeoffs →</a></p>
      ...
      <div className="notice"><strong>{capabilities?.mode === 'fixture' ? 'Fixture mode — synthetic configurations, not verified models.' : 'Live alpha verification is incomplete.'}</strong>Explicit examples replay recorded interpretations. You can request a disclosed local one-model proposal for diverse text tasks, or use an approved interpreting model. Nothing runs during planning. {capabilities?.live_gate}</div>
      ...
      <section id="describe"><h2>1. Describe and constrain</h2>
        ...
        <label className="check"><input type="checkbox" checked={draft.proposal_mode==='single_stage'} onChange={e=>setDraft({...draft,proposal_mode:e.target.checked?'single_stage':'interpreter'})} />Propose one model for this text task locally (reviewable template, no model-assisted interpretation)</label>
        <div className="form-grid">
          ...
          <label>Target deployment<select value={draft.requirements?.deployment ?? 'any'} onChange={e => requirements({deployment: e.target.value as S['TargetRequirements']['deployment']})}><option value="any">Any verified deployment</option><option value="self_hosted">Self-hosted only</option><option value="api">API only</option></select></label>
        </div>
      ...
      <section><h2>2. Processing and privacy — before any call</h2>
        ...
        <label>Catalog source<select value={draft.catalog_mode ?? 'fixture'} onChange={e => setDraft({...draft, catalog_mode: e.target.value as S['PlanInput']['catalog_mode']})}><option value="fixture" disabled={capabilities?.mode !== 'fixture'}>Synthetic fixture (invented configuration facts)</option><option value="public_snapshot">Retained public snapshot — September 6, offline</option><option value="runtime_public">Runtime official public metadata (no paid provider)</option><option value="approved_runtime" disabled={!capabilities?.runtime_catalog_ids?.length}>Approved runtime snapshot (same pin used by gateway)</option></select></label>
        {draft.catalog_mode==='approved_runtime'&&<label>Exact approved snapshot<select value={draft.runtime_catalog_id??''} onChange={e=>setDraft({...draft,runtime_catalog_id:e.target.value})}><option value="">Select a pinned snapshot</option>{capabilities?.runtime_catalog_ids?.map(id=><option key={id}>{id}</option>)}</select></label>}
      ...
      <section><h2>3. Clarification and edits</h2><p>Answers are saved with the version. Correct conflicting text and structured constraints explicitly; notes do not silently override a hard constraint.</p>
        ...
        <p className="hint">You can answer “unknown” without inventing a fact. Once an executable policy is saved, use its targeted edit proposal to keep a stage pinned while making another cheaper. Notes here never override hard constraints.</p>
